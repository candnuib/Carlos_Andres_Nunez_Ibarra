...
La evolución de la población en este experimento demostró cómo el entorno y las reglas genéticas moldean el comportamiento adaptativo. En primer lugar, los obstáculos actuaron como cuellos de botella evolutivos, forzando a los individuos a diversificar sus trayectorias y abandonar rutas directas que terminaban en bloqueos. Por su parte, la función de fitness fue la brújula direccional del sistema: al penalizar severamente los choques y premiar cada paso útil hacia el objetivo, moldeó un comportamiento progresivo y cauteloso que evitó que los robots se quedaran estancados en el punto de inicio. Finalmente, el operador de cruce actuó como un potente acelerador de la convergencia. Al permitir la recombinación de genes, la población pudo unir segmentos de rutas exitosas provenientes de distintos padres (por ejemplo, combinar el genoma que esquiva los primeros obstáculos con el que domina el tramo final), descubriendo la ruta óptima hacia la meta en muchas menos generaciones que utilizando únicamente mutaciones aleatorias.
